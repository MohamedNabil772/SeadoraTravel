    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
      <div class="space-y-4 bg-white p-6 rounded-xl border border-gray-200">
        ...
        <div v-for="(_, index) in (form.inclusions?.[currentLocale] || [])" :key="'inc-'+index" class="flex items-center gap-2">
          <input v-model="form.inclusions[currentLocale][index]" type="text" :aria-label="`Inclusion item ${Number(index) + 1}`" class="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500" placeholder="e.g. Hotel pickup" />
      ...
      <div class="space-y-4 bg-white p-6 rounded-xl border border-gray-200">
        ...
        <div v-for="(_, index) in (form.exclusions?.[currentLocale] || [])" :key="'exc-'+index" class="flex items-center gap-2">
          <input v-model="form.exclusions[currentLocale][index]" type="text" :aria-label="`Exclusion item ${Number(index) + 1}`" class="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500" placeholder="e.g. Gratuities" />
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div class="space-y-4">
          ...
          <div v-for="(_, index) in (form.importantInfo?.whatToBring?.[currentLocale] || [])" :key="'wtb-'+index" class="flex items-center gap-2">
            <input v-model="form.importantInfo.whatToBring[currentLocale][index]" type="text" :aria-label="`What to bring item ${Number(index) + 1}`" class="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500" placeholder="e.g. Passport" />
        ...
        <div class="space-y-4">
          ...
          <div v-for="(_, index) in (form.importantInfo?.notSuitableFor?.[currentLocale] || [])" :key="'nsf-'+index" class="flex items-center gap-2">
            <input v-model="form.importantInfo.notSuitableFor[currentLocale][index]" type="text" :aria-label="`Not suitable for item ${Number(index) + 1}`" class="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500" placeholder="e.g. Wheelchair users" />
        ...
        <div class="space-y-4 md:col-span-2">
          ...
          <div v-for="(_, index) in (form.importantInfo?.notes?.[currentLocale] || [])" :key="'note-'+index" class="flex items-center gap-2">
            <input v-model="form.importantInfo.notes[currentLocale][index]" type="text" :aria-label="`Note item ${Number(index) + 1}`" class="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500" placeholder="e.g. Please wait in the hotel lobby" />
...
const addInclusion = () => {
  ensureReady()
  form.value.inclusions[currentLocale.value].push('')
}
const addExclusion = () => {
  ensureReady()
  form.value.exclusions[currentLocale.value].push('')
}
const addWhatToBring = () => {
  ensureReady()
  form.value.importantInfo.whatToBring[currentLocale.value].push('')
}
const addNotSuitableFor = () => {
  ensureReady()
  form.value.importantInfo.notSuitableFor[currentLocale.value].push('')
}
const addNote = () => {
  ensureReady()
  form.value.importantInfo.notes[currentLocale.value].push('')
}
</script>
